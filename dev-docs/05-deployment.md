...
1. **自动计算 base path**（"Compute base path" 步骤）：仓库名为 `*.github.io`
   或存在 `CNAME`（用户/组织页面或自定义域名，根路径）→ 留空；否则（项目页面）
   → `/<repo>`。
2. `npm ci` → `npm run build`，`NEXT_PUBLIC_BASE_PATH` 取上一步结果。
3. `touch out/.nojekyll`（禁用 Jekyll，让 `_next/` 原样提供）。
4. `upload-pages-artifact` + `deploy-pages` 发布。

> 早期版本把 base path 硬编码为 `/<repo>`，会导致用户/组织页面（根路径部署）的
> 资源 404、整页无样式。现已自动判断。
